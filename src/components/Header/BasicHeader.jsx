...
export default function BasicHeader({ mode, isFollowersPage, followMode }) {
    ...
    return (
        <HeaderContainer>
            <ArrowStyle>
                ...
                {!isFollowersPage ? <UserName>{mode === 'post' ? '' : user.username}</UserName> : <UserName>{followMode}</UserName>}
            </ArrowStyle>
            {!isFollowersPage && <IconImg src={moreIcon} alt="더보기 아이콘" onClick={moreIconHandle} />}
        </HeaderContainer>
    );
}
